...
NUM_K_FOLD = 5
EPOCH = 10
AVERAGE = 0.9995 # 0.0 => no-average, 1.0 => naive average, 0.0<value<1.0 => moving average

SP_VOCAB_SIZE = 2048
DIM_SP_EMB = 256
DIM_SP_CELL_SIZE = 128
SP_USE_EPOCH = 10
SP_BATCH_SIZE = 512
SP_AVERAGE = False
SP_DROPOUT = False
SP_DO_VALIDATE = False
SP_NUM_K_FOLD = 5
SP_MODEL_NAME = "lmmodel/{}_emb{}_cell{}{}{}{}{}/model".format(
    "validate{}".format(SP_NUM_K_FOLD if SP_NUM_K_FOLD != 5 else "") if SP_DO_VALIDATE else "all",
    DIM_SP_EMB, DIM_SP_CELL_SIZE,
    "_dropout" if SP_DROPOUT else "",
    "_average" if SP_AVERAGE else "",
    "_batch{}".format(SP_BATCH_SIZE) if SP_BATCH_SIZE != 512 else "",
    "_size{}".format(SP_VOCAB_SIZE) if SP_VOCAB_SIZE != 2048 else "")
...
preprocessed = np.load("preprocessed-sp{}.npy".format(SP_VOCAB_SIZE)).tolist()
...
# SP
sp_bos_id, sp_eos_id = preprocessed["sp_bos_eos"]
for instances in [all_train_instances, test_instances]:
    with ctqdm(instances, desc="flat sp") as tinstances:
        for instance in tinstances:
            words = instance["word"]
            sp = instance["sp"]
            new_words = []
            for sp_word, word in zip(sp, words):
                new_word = [word] + [0] * (len(sp_word)-1)
                #new_word = [word] * len(sp_word)
                new_words.append(new_word)
            new_words = sum(new_words, [])[:200]
            instance["word"] = new_words
            instance["sequence_length"] = len(new_words)
            instance["sp"] = [sp_bos_id] + sum(sp, [])[:200] + [sp_eos_id]
...
# SP
if params.lm_epoch > 0:
    all_instances = np.concatenate([all_train_instances, test_instances], axis=0)
    if SP_DO_VALIDATE:
        all_labels = [instance["label"] for instance in all_instances]
        sp_skfold = StratifiedKFold(SP_NUM_K_FOLD, shuffle=True, random_state=12345)
        sp_lm_train_ids, sp_lm_val_ids = list(sp_skfold.split(all_labels, all_labels))[0]
        sp_train_gen = Gen(all_instances, batch_size=SP_BATCH_SIZE, shuffle=True).construct(sp_lm_train_ids)
        sp_val_gen = Gen(all_instances, batch_size=SP_BATCH_SIZE, shuffle=True).construct(sp_lm_val_ids)
    else:
        sp_train_gen = Gen(all_instances, batch_size=SP_BATCH_SIZE, shuffle=True).construct()
...
# SP
if params.lm_epoch > 0:
    sp_train_dataset = tf.data.Dataset.from_generator(sp_train_gen, output_shapes=sp_train_gen.output_shapes, output_types=sp_train_gen.output_types).prefetch(8)
    init_sp_train = dataset_iterator.make_initializer(sp_train_dataset)
    if SP_DO_VALIDATE:
        sp_val_dataset = tf.data.Dataset.from_generator(sp_val_gen, output_shapes=sp_val_gen.output_shapes, output_types=sp_val_gen.output_types).prefetch(8)
        init_sp_val = dataset_iterator.make_initializer(sp_val_dataset)
...
# SP
sp_emb = tf.keras.layers.Embedding(SP_VOCAB_SIZE, DIM_SP_EMB)
h_sp_emb = sp_emb(minibatch["sp"][:,1:-1])
sp_is_training = tf.placeholder_with_default(False, [])
if SP_DROPOUT:
    h_sp_emb = tf.cond(sp_is_training, lambda: tf.nn.dropout(h_sp_emb, 0.9), lambda: h_sp_emb)
sp_flstm1 = tf.keras.layers.CuDNNLSTM(DIM_SP_CELL_SIZE, return_sequences=True)
sp_blstm1 = tf.keras.layers.CuDNNLSTM(DIM_SP_CELL_SIZE, return_sequences=True)
def sp_reverse(i): return tf.reverse_sequence(i, seq_lens, batch_axis=0, seq_axis=1)
h_sp_flstm1 = sp_flstm1(h_sp_emb)
h_sp_blstm1 = sp_reverse(sp_blstm1(sp_reverse(h_sp_emb)))
sp_fc_output = tf.keras.layers.Dense(SP_VOCAB_SIZE)
if SP_DROPOUT:
    sp_flogits = sp_fc_output(tf.cond(sp_is_training, lambda: tf.nn.dropout(h_sp_flstm1, 0.9), lambda: h_sp_flstm1))
    sp_blogits = sp_fc_output(tf.cond(sp_is_training, lambda: tf.nn.dropout(h_sp_blstm1, 0.9), lambda: h_sp_blstm1))
else:
    sp_flogits = sp_fc_output(h_sp_flstm1)
    sp_blogits = sp_fc_output(h_sp_blstm1)
sp_fgold = minibatch["sp"][:,2:]
sp_bgold = minibatch["sp"][:,:-2]
sp_mask = tf.sequence_mask(seq_lens, tf.shape(minibatch["word"])[1], dtype=tf.float32)
sp_floss = tf.losses.sparse_softmax_cross_entropy(labels=sp_fgold, logits=sp_flogits, weights=sp_mask)
sp_bloss = tf.losses.sparse_softmax_cross_entropy(labels=sp_bgold, logits=sp_blogits, weights=sp_mask)
sp_loss = sp_floss + sp_bloss
sp_mean_loss, sp_update_mean_loss = tf.metrics.mean(sp_loss)

sp_variables = []
sp_variables.extend(sp_emb.variables)
sp_variables.extend(sp_flstm1.variables)
sp_variables.extend(sp_blstm1.variables)
sp_variables.extend(sp_fc_output.variables)
...
class Model:
    ...
    def __init__(self):
        ...
        # SP
        #word_emb = word_emb * tf.cast(tf.not_equal(word_ids, 0), tf.float32)[:,:,tf.newaxis]

        word_vec = word_emb

        # SP
        w = tf.Variable(1.0, name="sp_weight")
        #word_vec = tf.concat([word_vec, tf.stop_gradient(tf.concat([h_sp_flstm1, h_sp_blstm1], axis=-1))], axis=-1)
        #word_vec = tf.concat([word_vec, w * tf.stop_gradient(tf.concat([h_sp_flstm1, h_sp_blstm1], axis=-1))], axis=-1)
        #word_vec = tf.concat([word_vec, tf.stop_gradient((tf.concat([h_sp_flstm1, h_sp_blstm1], axis=-1) + h_sp_emb)/ 2.0)], axis=-1)
        word_vec = tf.concat([word_vec, w * tf.stop_gradient((tf.concat([h_sp_flstm1, h_sp_blstm1], axis=-1) + h_sp_emb)/ 2.0)], axis=-1)
        ...
        h_bilstm1, h_flstm1, h_blstm1 = bilstm(word_vec, self.flstm1, self.blstm1, return_all=True)
        if self.use_skip1:
            prev = word_vec
            weight = self.w_skip1
        ...
        """
        if params.lm_epoch > 0:
            self.layer_fc_lm1_f_hidden = tf.keras.layers.Dense(params.dim_lm_hidden)
            self.layer_fc_lm1_b_hidden = tf.keras.layers.Dense(params.dim_lm_hidden)
            h_lm1_f_hidden = tf.nn.tanh(self.layer_fc_lm1_f_hidden(h_flstm1))
            h_lm1_b_hidden = tf.nn.tanh(self.layer_fc_lm1_b_hidden(h_blstm1))
            self.layer_fc_lm1 = tf.keras.layers.Dense(len(initial_char_to_id))
            lm1_f_logits = self.layer_fc_lm1(dropout(h_lm1_f_hidden, 0.5))
            lm1_b_logits = self.layer_fc_lm1(dropout(h_lm1_b_hidden, 0.5))
            self.lm_logits = [lm1_f_logits, lm1_b_logits]
        """
...
# SP
sp_train_op = opt.minimize(sp_loss)
...
def do_lm(max_epoch):
    sp_saver = tf.train.Saver(sp_variables, max_to_keep=1000)
    min_val_loss = np.inf
    for epoch in range(max_epoch):
        feed = params.feed({is_training:True, sp_is_training:True})
        sess.run(tf.local_variables_initializer())
        sess.run(init_sp_train)
        train_sum_loss = 0.0
        with contextlib.suppress(tf.errors.OutOfRangeError), contextlib.closing(tqdm.tqdm(total=sp_train_gen.get_target_len())) as pbar:
            while True:
                l, _, _, w = sess.run([sp_loss, sp_update_mean_loss, sp_train_op, word_ids], feed_dict=feed)
                if SP_AVERAGE: average_manager.update_averages(sess)
                bsize = len(w)
                train_sum_loss += l * bsize
                if pbar.n % 1000 == 0: pbar.set_description("loss:{loss:.4e}".format(loss=l))
                pbar.update(bsize)
        train_mean_loss = sess.run(sp_mean_loss)

        if SP_AVERAGE:
            average_manager.switch_to_average(sess)
        if SP_DO_VALIDATE:
            feed = params.feed({is_training:False, sp_is_training:False})
            sess.run(tf.local_variables_initializer())
            sess.run(init_sp_val)
            val_sum_loss = 0.0
            with contextlib.suppress(tf.errors.OutOfRangeError), contextlib.closing(tqdm.tqdm(total=sp_val_gen.get_target_len())) as pbar:
                while True:
                    l, _, w = sess.run([sp_loss, sp_update_mean_loss, word_ids], feed_dict=feed)
                    bsize = len(w)
                    val_sum_loss += l * bsize
                    if pbar.n % 1000 == 0: pbar.set_description("loss:{loss:.4e}".format(loss=l))
                    pbar.update(bsize)
            val_mean_loss = sess.run(sp_mean_loss)
            print("lm epoch:{}   train_loss:{}({})   val_loss:{}({})".format(epoch, train_sum_loss, train_mean_loss, val_sum_loss, val_mean_loss))
        else:
            print("lm epoch:{}   train_loss:{}({})".format(epoch, train_sum_loss, train_mean_loss))

        #if min_val_loss > val_mean_loss:
        if True:
            if SP_DO_VALIDATE:
                min_val_loss = val_mean_loss
            sp_saver.save(sess, SP_MODEL_NAME, global_step=epoch)
            print("saved") 
        if SP_AVERAGE:
            # switch to temporal after saving.
            average_manager.switch_to_temporal(sess)
    return
...
def run(params, do_write, name_suffix=None):
    ...
    with contextlib.ExitStack() as stack:
        ...
        sess.run(tf.global_variables_initializer())
        sp_saver = tf.train.Saver(sp_variables)
        sp_saver.restore(sess, SP_MODEL_NAME + "-{}".format(SP_USE_EPOCH))
...
if SUBMIT:
    ...
    run(params=params, do_write=True, name_suffix="spf"+(sys.argv[1] if len(sys.argv) == 2 else ""))
