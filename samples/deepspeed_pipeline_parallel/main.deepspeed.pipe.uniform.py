...
HIDDEN_DIMS = [512, 4096, 8192, 16384, 32768, 32768, 32768, 32768, 32768, 32768, 32768, 32768, 32768, 32768, 32768, 16384, 8192, 8192, 4096] # not fit 2x A100 w/o pipeline parallelism
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        if args.reduce_memory and DO_PIPELINE:
            ...
            self.layers.append(deepspeed.pipe.LayerSpec(LinearWithPrint, INPUT_DIM, 512))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Identity))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 512, 4096))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 4096, 8192))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 8192, 16384))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 16384, 32768))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 32768, 32768))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 32768, 32768))
            ...
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 32768, 32768))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 32768, 32768))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 32768, 16384))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 16384, 8192))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 8192, 8192))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 8192, 4096))
            self.layers.append(deepspeed.pipe.LayerSpec(torch.nn.Linear, 4096, OUTPUT_DIM))
            ...
            self.layers.append(LinearWithPrint(INPUT_DIM, 512))
            self.layers.append(torch.nn.Identity())
            self.layers.append(torch.nn.Linear(512, 4096))
            self.layers.append(torch.nn.Linear(4096, 8192))
            self.layers.append(torch.nn.Linear(8192, 16384))
            self.layers.append(torch.nn.Linear(16384, 32768))
            self.layers.append(torch.nn.Linear(32768, 32768))
            self.layers.append(torch.nn.Identity())
            ...
            self.layers.append(torch.nn.Linear(32768, 32768))
            self.layers.append(torch.nn.Linear(32768, 32768))
            self.layers.append(torch.nn.Linear(32768, 16384))
            self.layers.append(torch.nn.Linear(16384, 8192))
            self.layers.append(torch.nn.Linear(8192, 8192))
            self.layers.append(torch.nn.Linear(8192, 4096))
            self.layers.append(torch.nn.Linear(4096, OUTPUT_DIM))
...
if DO_PIPELINE:
    model = deepspeed.pipe.PipelineModule(layers=model.layers, num_stages=args.pipeline_size, loss_fn=loss_fn, partition_method="uniform")
